...
class VTKLoader extends Loader {
    ...
    parse( data ) {
        ...
        function parseASCII( data ) {
            ...
            for ( var i in lines ) {
                ...
                if ( line.indexOf( 'DATASET' ) === 0 ) {
                    ...
                    if ( dataset !== 'UnstructuredGrid' ) throw new Error( 'Unsupported DATASET type: ' + dataset );
        ...
        function parseXML( stringFile ) {
            ...
            // MARK: - Start of customized VTU Loader Code (most changes are annotated)
            // Aggregation variables for input
            var pointData = {};
            var dataComponents = [];

            // Import from UnstructuredGrid instead of PolyData
            if ( json.UnstructuredGrid ) {

                var piece = json.UnstructuredGrid.Piece;
                var compressed = json.attributes.hasOwnProperty( 'compressor' ); // Can be optimized
                // Loop through the sections

                // Only get two sections PointData and Points
                    ...
                    if ( section && section.DataArray ) {
                        ...
                        while ( dataArrayIndex < numberOfDataArrays ) {
                            ...
                            if ( '#text' in arr[ dataArrayIndex ] && arr[ dataArrayIndex ][ '#text' ].length > 0 ) {
                                ...
                                const parsedDate = parseDataArray( arr[ dataArrayIndex ], compressed );
                                arr[ dataArrayIndex ].text = parsedDate;
                        ...
                        switch ( sections[ sectionIndex ] ) {
                            ...
                            case 'PointData':
                                ...
                                var vectorsName  = section.attributes.Vectors; // Added vector name
                                if ( numberOfPoints > 0 && normalsName) {
                                    ...
                                    // Added handling for vector name (this is used in VTU files)
                                    const vectorArray = section.DataArray
                                    for ( var i = 0, len = vectorArray.length; i < len; i++) {
                                        // Add every data component to aggregator
                                        const name = vectorArray[i].attributes.Name
                                        var components = parseInt(vectorArray[i].attributes.NumberOfComponents || "1")
                                        const valueArray = vectorArray[i].text;
                                        pointData[name] = valueArray
                                        dataComponents.push(components)
                                    }
                                }
                ...
                // Replaced here creation of geometry by return of aggregation result data
                const result = {
                    points,
                    pointData
                }
